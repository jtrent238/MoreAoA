						...
						for (int k2 = 0; k2 < 4; ++k2) {
							...
							for (int l2 = 0; l2 < 4; ++l2) {
								if ((lvt_45_1_ += d16) > 0.0D) {
									primer.setBlockState(i * 4 + k2, i2 * 8 + j2, l * 4 + l2, ModBlocks.dirtSlime.getDefaultState());
								}
								else if (i2 * 8 + j2 == world.getSeaLevel()) {
									// TODO: add slime liquid
	...
	private void generateBiomeTerrain(World worldIn, Random rand, ChunkPrimer chunkPrimerIn, int x, int z, double noiseVal) {
		...
		for (int j1 = 255; j1 >= 0; --j1) {
			if (j1 <= 2) {
				...
				else if (iblockstate2.getBlock() == ModBlocks.dirtSlime) {
					if (j == -1) {
						if (k <= 0) {
						...
						if (j1 < seaLevel && (topBlock == null || topBlock.getMaterial() == Material.AIR)) {
							if (biome.getTemperature(tempCheckPos.setPos(x, j1, z)) < 0.15F) {
								...
								topBlock = ModBlocks.dirtSlime.getDefaultState();
							}
						}
